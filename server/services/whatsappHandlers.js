...
const { getReputation, getRewards, claimRewards, getWalletBalance, fundWallet } = require('./blockchain');
...
const handleIncomingMessage = async (params) => {
  ...
  if (command === 'REPORT') {
    ...
    if (!user) {
      ...
      logger.info(`[WhatsApp Handler] Created new user with wallet: ${address}`);
      
      // Try to fund wallet with some ETH for gas (if deployer has funds)
      try {
        await fundWallet(address, '0.001');
        logger.info(`[WhatsApp Handler] Funded wallet with 0.001 ETH`);
      } catch (e) {
        logger.error(`[WhatsApp Handler] Could not fund wallet: ${e.message}`);
      }
    }
